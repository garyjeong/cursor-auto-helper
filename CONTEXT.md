...
Cursor Auto Resumer는 Cursor IDE에서 Agent 채팅 패널의 Resume 버튼을 자동으로 감지하고 클릭하는 VSCode 확장입니다. 이 확장은 사용자가 Agent와의 대화를 중단 없이 계속할 수 있도록 도와줍니다.
...
│   ├── config.ts          # 설정 관리 클래스
│   ├── webviewScript.ts   # 웹뷰 DOM 모니터링 스크립트
│   ├── agentWatcher.ts    # 메인 감시 로직
│   └── extension.ts       # 확장 진입점
├── tests/
